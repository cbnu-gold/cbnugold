...
- 검증된 운영 패키지는 `초안으로 불러오기`로 편집 화면에만 반영할 수 있습니다. 모든 콘텐츠는 `draft`로 들어오고 모집은 닫힌 상태가 되며, `공개 CMS 전체 저장` 또는 각 항목 저장으로 DB에 반영합니다.
- 프리셋 또는 운영 패키지 초안을 불러올 때 기존 페이지 slug, 블록 page/block key, 모집 기수, 활동 category/title, 성과 kind/title/result, 연혁 year, FAQ question이 일치하면 기존 항목의 `id`를 보존합니다. 따라서 `공개 CMS 전체 저장`은 중복 생성 대신 기존 항목 업데이트로 처리됩니다.
- `공개 CMS 전체 저장`은 현재 편집 화면의 사이트 설정, 페이지, 콘텐츠 블록, 모집, 활동, 성과, 연혁, FAQ를 저장합니다. 지원자, 관리자 계정, 감사 로그, 비공개 지원서 파일, 미디어 스토리지 파일은 저장 대상이 아닙니다.
